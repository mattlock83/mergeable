...
class Checks extends Action {
  ...
  async afterValidate (context, settings, name, results) {
    ...
    if (process.env.IS_GITHUB_ACTION === 'true') {
      // Exit with 1 if checks fail via Github actions
      // Needed as checks aren't reported on PR status except on push or pull_request event
      // See https://github.blog/changelog/2019-09-24-ui-changes-in-github-actions-checks/
      const core = require('@actions/core')
      const validationErrors = []
      if (settings.status === 'failure') {
        results.validationSuites.forEach(val => {
          val.validations.forEach(el => {
            // Grab the message for each failed validation for reporting
            if (el.status === 'fail') {
              validationErrors.push(el.description + '\n')
            }
          })
        })
        core.setFailed('Mergeable check failed!' + '\n' + validationErrors)
      }
    }
  }
}
